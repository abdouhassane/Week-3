...
```{r}
ggplot(data = plastic_waste, aes(x = plastic_waste_per_cap), colour = continent) +
  geom_histogram(binwidth = 0.2) +
...
```{r}
#geom_density affects the whole graph aesthetic but using ggplot each variable is affected differently giving us more customisation for each variable.
```
...
ggplot(data = plastic_waste, 
       mapping = aes(x = continent, 
                     y = plastic_waste_per_cap)) +
  geom_violin()
#box plots we can view the median and iQR ranges and see where the outliers are.
#violin plots: easier distribution view to see datasets near the median, more continuous and point in the dataset is connected.
```
...
ggplot(data = plastic_waste,
       mapping = aes(x = mismanaged_plastic_waste_per_cap, 
                     y = plastic_waste_per_cap)) +
  geom_point()
#weak positive correlation, there is one outlier
```
...
ggplot(data = plastic_waste,
  ...
  geom_point()
# European continent seem to be better at managing plastic waste, while Asian, oceania, Africa have few countries with high mismanagedplastic waste per capita
```
...
# no strong association between either, plots are very similar and not distinct.
```
