...
```{r}
library(tidyverse)
new_vax <- world_vaccination %>% 
filter(!is.na(pct_vaccinated), who_region != ("(WHO) Global"))
```

Hint: When you want to filter for rows that aren't equal to something, in R you can use the N!= operator. For example, to remove all the cars with 6 cylinders from the mtcars built-in R data set we would do the following:
```{r}
filter(mtcars, cyl != 6)
```
...
```{r}
world_vacc_plot <- new_vax %>%  
  ggplot(aes(x = yr, y = pct_vaccinated, color = who_region)) +
  geom_point() +
  facet_wrap(~ vaccine) +
  labs(x = "year", y = "percentage of people vaccinated", title = "World Vaccination Plot")
...
```{r}
world_vacc_plot2 <- new_vax %>%  
  ggplot(aes(x = yr, y = pct_vaccinated, color = vaccine, shape = vaccine)) +
  geom_point() +
  labs(x = "year", y = "percentage of people vaccinated", title = "World Vaccination Plot")
...
```{r}
polio_regions <- polio %>% 
  ggplot(aes(x = yr, y = pct_vaccinated, color = who_region, shape = who_region)) +
  geom_point()
